...
The engine displays/prints the normalized candidate manifest with sanitized source/evidence, operation confidence where document-derived, and deterministic `digest`. The operator must pass that exact digest to both `approve` and `generate`; after output-target validation succeeds, the engine consumes a state-local HMAC receipt bound to the digest before rendering. Its fields are `manifest_digest`, `issued_at`, `expires_at`, `nonce`, and `hmac`; it expires and is single-use. Missing, stale, used, tampered, or unbound receipts stop generation. This is a local generation gate, not an audit trail or a substitute for user confirmation immediately before a separate GitHub publication. The engine never installs dependencies, calls an upstream API, creates a GitHub repository, or pushes output before these gates are valid.
