...
7. **Contained execution.** Generation and generated-project verification run in a fresh temporary workspace with a scrubbed environment and explicit network policy. Inspect `package.json` and the generated `package-lock.json`, then run `npm ci --ignore-scripts` before any lifecycle script. The verifier disables inherited npm proxy configuration and must document that host-level container/VM/sandbox isolation remains the caller's responsibility. Live authenticated smoke tests require separate explicit approval.
