...
All contract failures return a fixed safe code/message pair: `MANIFEST_SCHEMA_INVALID`, `MANIFEST_DIGEST_MISMATCH`, or `CANONICALIZATION_FAILED`. Canonicalization rejects non-finite numbers, invalid Unicode, sparse arrays, cyclic values, non-JSON values, and property-access failures without exposing the original exception. These library APIs do not persist data, issue challenges/receipts, construct a manifest from an API source, or generate files.
